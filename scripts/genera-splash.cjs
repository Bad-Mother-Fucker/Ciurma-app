...
function pixel(width, height, x, y) {
  const cx = width / 2;
  const cy = height / 2;
  const scala = Math.min(width, height) * 0.16;

  const anelloR = scala;
  const anelloSpessore = scala * 0.16;
  const centroR = scala * 0.2;
  const razzaSpessore = scala * 0.14;
  const razzaInterno = scala * 0.4;
...
function makePng(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const colore = pixel(width, height, x + 0.5, y + 0.5);
...
fs.writeFileSync(process.argv[2], makePng(parseInt(process.argv[3], 10), parseInt(process.argv[4], 10)));
